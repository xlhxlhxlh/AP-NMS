...
$t = $c_echo[0];
$b = $t + 1;
$test = mysqli_query($link,"select * from ap_server where id ='{$id}' ");
// 结果集
$tar = mysqli_fetch_array($test);
$Apserver_IP = $tar[1];

mysqli_query($link,"insert into control(id,object,number,operation,time,state,content) values ('{$b}','2','1','1','{$k}','0','{$Apserver_IP}')") or die('添加数据出错：'.mysqli_error());  
mysqli_query($link,"DELETE FROM ap_server WHERE id={$id}") or die('删除数据出错：'.mysqli_error());
// 删除完跳转到新闻页
